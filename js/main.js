...
document.addEventListener('click',e=>{   const pm=document.getElementById('plusMenu'),pb=document.getElementById('plusBtn');   const pp=document.getElementById('pickerPanel'),eb=document.getElementById('emojiBtn');   const cm=document.getElementById('ctxMenu');   const rw=document.getElementById('reactPickerWrap');   const pv=document.getElementById('pastePreview');   if(state.plusOpen&&!pm.contains(e.target)&&!pb.contains(e.target))closePlus();   if(state.pickerOpen&&!pp.contains(e.target)&&!eb.contains(e.target))closePicker();   if(state.ctxTarget&&!cm.contains(e.target))closeCtxMenu();   if(state.reactTargetId&&!rw.contains(e.target))closeReactPicker(); }); document.addEventListener('contextmenu',e=>{   if(!e.target.closest('.bubble')&&!e.target.closest('#ctxMenu'))closeCtxMenu(); });
